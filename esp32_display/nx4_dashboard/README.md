...
├──────────────┼──────────────┤                        180         │
│▌09/01 週一   │▌道路速限     │                          ┌────────┐│
│  18:04       │  90          │      +0.15  BAR          │測速 90 ││
│              │              │  ▁▁▁▁▁▁▁┃▁▁▁▁▁▁          └────────┘│
│              │              │  -1  -0.5  0  +0.5  +1   ● LINK 近燈│
│              │              │                          10.0.4.99 │
└──────────────┴──────────────┴────────────────────────────────────┘
```
...
**右下角狀態區**取代 rec.gif 中 App 的按鈕列，顯示連線狀態、IP、
螢幕亮度、大燈狀態，以及測速照相警示（紅色膠囊 500 ms 閃爍）。
擺在右下角而非右側整條，是為了把整個右半部讓給時速環；
增壓區因此略為偏左（`TURBO_CX`），避開右下角的狀態區。
