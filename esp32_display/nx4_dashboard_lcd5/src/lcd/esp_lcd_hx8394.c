...
#define HX8394_CMD_GS_BIT       (1 << 0)
#define HX8394_CMD_SS_BIT       (1 << 1)
...
static esp_err_t panel_hx8394_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y);
                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    panel_handle->mirror = panel_hx8394_mirror;
...
// ─────────────────────────────────────────────────────────────────────────
// 原廠 720x1280 2-lane 初始化序列。
// 取自 waveshareteam/ESP32-P4-WIFI6-Touch-LCD-5 的
// examples/arduino/libraries/displays/displays_config.h（vendor_specific_init_default）。
//
// 該表開頭的 0x11 / 0x36 / 0x3A / 0xBA 四筆屬於「前置命令」，本驅動的
// panel_hx8394_init() 已自行送出（SLPOUT、MADCTL、COLMOD、DSI 2-lane），
// 故此處從 0xB9（SETEXTC 解鎖）開始，避免與 madctl_val 打架。
// ─────────────────────────────────────────────────────────────────────────
static const hx8394_lcd_init_cmd_t vendor_specific_init_default[] = {
...
static esp_err_t panel_hx8394_init(esp_lcd_panel_t *panel)
{
    ...
    // 0xBA bit[1:0] = 01 → 2 data lanes
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, 0xBA, (uint8_t[]) {
        0x61,
    }, 1), TAG, "send command failed");
    ...
    if (hx8394->init_cmds) {
        ...
        init_cmds_size = sizeof(vendor_specific_init_default) / sizeof(hx8394_lcd_init_cmd_t);
    }
...
static esp_err_t panel_hx8394_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    hx8394_panel_t *hx8394 = (hx8394_panel_t *)panel->user_data;
    esp_lcd_panel_io_handle_t io = hx8394->io;
    uint8_t madctl_val = hx8394->madctl_val;

    ESP_RETURN_ON_FALSE(io, ESP_ERR_INVALID_STATE, TAG, "invalid panel IO");

    // Control mirror through LCD command
    if (mirror_x) {
        madctl_val |= HX8394_CMD_GS_BIT;
    } else {
        madctl_val &= ~HX8394_CMD_GS_BIT;
    }
    if (mirror_y) {
        madctl_val |= HX8394_CMD_SS_BIT;
    } else {
        madctl_val &= ~HX8394_CMD_SS_BIT;
    }
